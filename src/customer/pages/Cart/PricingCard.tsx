...
const PricingCard = ({ guestCart = null }: PricingCardProps) => {
  const { cart } = useAppSelector((state) => state.cart);

  const isGuest = !!guestCart;

  /* ===================== GUEST CALCULATIONS ===================== */

  const guestSubtotal = isGuest
    ? guestCart.reduce(
  ...
  const guestFinalAmount = isGuest
    ? guestCart.reduce(
      (total: number, item: any) =>
        total +
  ...
  const guestDiscount = guestSubtotal - guestFinalAmount;
  ...
  const discount = isGuest
    ? guestDiscount
    : cart?.discount || 0;

  const couponDiscount = isGuest
    ? 0
