...
const Order = () => {
  ...
  const {success: slipSuccess, error: slipError,} = useAppSelector((state)=>state.packingSlip)

  const {success: replacementSuccess, error: replacementError,} = useAppSelector((state)=>state.replacementSeller)
  ...
  useEffect(() => {
    ...
    if(slipSuccess){
      setSnackbar({
        open: true,
        message: "Packing Slip downloaded successfully",
        severity: "success",
      });
    }
  if(slipError){
...
useEffect(() => {
  if(replacementSuccess){
    setSnackbar({
      open: true,
      message: "Relacement action completed successfully",
      severity: "success"
    })
  }
